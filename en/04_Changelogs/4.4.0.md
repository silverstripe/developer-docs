...
- [Upgrade to React 16 in the CMS](#upgrade-to-react-16-in-the-cms)
...
1. Add a new `extra.resources-dir` key to your composer file.
...
1. Expose your vendor assets by running `composer vendor-expose`.
1. Remove the old `resources` folder. This folder will be located in the `public` folder if you have adopted the public web root, or in the root of your project if you haven't.
...
Pages and other views can contain links to asset locations, e.g.
as HTML content rendered with `<img>` and `<a>` tags.
...
Yes. Simply disable `legacy_filenames` in your YAML configuration, then run the `MigrateFileTask`. This will normalise
...
### `DevelopmentAdmin` controllers {#upgrading-developmentadmin-controllers}
