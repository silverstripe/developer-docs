...
There is also additional configuration, such as which `SearchFilter` it uses ([PartialMatchFilter](api:SilverStripe\ORM\Filters\PartialMatchFilter) by default). Check out [General search field](/developer_guides/model/scaffolding#general-search-field) for more information.
