...
A full list of module versions included in CMS Recipe 6.0.0 is provided below. We recommend referencing recipes in your dependencies, rather than individual modules, to simplify version tracking. See [Recipes](/getting_started/).
...
To facilitate swapping to a new editor, we have further abstracted the [`HTMLEditorConfig`](api:SilverStripe\Forms\HTMLEditor\HTMLEditorConfig) class and related functionality. This change makes it easier to replace TinyMCE with a different HTML editor in the future.

Part of this change includes a new way to define rules such as which elements and attributes are allowed in HTML content. You can continue to set the `valid_elements` and `extended_valid_elements` configuration on a [`TinyMCEConfig`](api:SilverStripe\TinyMCE\TinyMCEConfig) object if you want to, but the new API is generic and will work for any `HTMLEditorConfig` implementation and can be defined in YAML configuration.
...
Part of this upgrade includes a change to assertions which expect warnings, errors, and notices. See [asserting errors, warnings and notices](/developer_guides/testing/unit_testing/#asserting-errors-warnings-and-notices) for details.
...
We've upgraded the various Symfony dependencies from v6 to v7, as per our [fixes dependencies policy](/project_governance/fixed_dependencies/).
...
[`DBField`](api:SilverStripe\ORM\FieldType\DBField) is the base class for all database fields in Silverstripe CMS. For instance when you define `'MyField' => 'Varchar(255)'` in your [`DataObject`](api:SilverStripe\ORM\DataObject) subclass, the `MyField` property will be backed by an instance of [`DBVarchar`](api:SilverStripe\ORM\FieldType\DBVarchar).
...
The validation is added through subclasses of the new [`FieldValidator`](api:SilverStripe\Core\Validation\FieldValidation\FieldValidator) abstract class, for instance the [`StringFieldValidator`](api:SilverStripe\Core\Validation\FieldValidation\StringFieldValidator) is used to validate [`DBVarchar`](api:SilverStripe\ORM\FieldType\DBVarchar). See [data types and casting](/developer_guides/model/data_types_and_casting/#available-types) for a full list of validation rules.
...
- [`DBDecimal`](api:SilverStripe\ORM\FieldType\DBDecimal) and its subclasses will convert numeric strings as well as integers to floats.
...
>
> You may wish to create a [`BuildTask`](api:SilverStripe\Dev\BuildTask) that calls [`DataObject::validate()`](api:SilverStripe\ORM\DataObject::validate()) on all affected records to identify what data needs to be updated.
...
- Write a `BuildTask` that updates any invalid values in your database (see [`BuildTask` documentation](/developer_guides/cli/polycommand/#buildtask)).
...
- [`DBPercentage`](api:SilverStripe\ORM\FieldType\DBPercentage) stores values internally as a float. It will no longer convert values above `1.0` such as `2.3` to `1.0` when calling `setValue()`. Instead, it will keep the value as `2.3`. If the value exceeds the value of [`DBPercentage::getMaxValue()`](api:SilverStripe\ORM\FieldType\DBPercentage::getMaxValue()), which has a default value of `1.0`, then it will fail validation.
...
[`ValidationResult`](api:SilverStripe\Core\Validation\ValidationResult) objects created by [`DataObject::validate()`](api:SilverStripe\ORM\DataObject::validate()) now include additional information about validation exceptions, including the field name that failed validation, and if applicable the model class and record ID.
...
This additional information will be included in the validation message by [`ValidationException`](api:SilverStripe\Core\Validation\ValidationException) if:

- the validation exception happened in a CLI context, or
- the validation exception happened in an HTTP context and the current controller is an instance or subclass of one of the classes listed in the new [`ValidationException.show_additional_info_non_cli_controllers`](api:SilverStripe\Core\Validation\ValidationException->show_additional_info_non_cli_controllers) configuration.
...
By default only [`DevelopmentAdmin`](api:SilverStripe\Dev\DevelopmentAdmin) and its subclasses are configured to show additional information in an HTTP context.

### Changes to password validation {#password-validation}

#### `PasswordValidator` changes

The `SilverStripe\Security\PasswordValidator` class (which used to be the default password validator) has been renamed to [`RulesPasswordValidator`](api:SilverStripe\Security\Validation\RulesPasswordValidator) and is now optional.

A new [`EntropyPasswordValidator`](api:SilverStripe\Security\Validation\EntropyPasswordValidator) class has been created, and is now the default password validator. `EntropyPasswordValidator` is powered by the [`PasswordStrength` constraint](https://symfony.com/doc/current/reference/constraints/PasswordStrength.html) in `symfony/validator`. This constraint determines if a password is strong enough based on its entropy, rather than on arbitrary rules about what characters it contains.
...
See the [sake documentation](/developer_guides/cli/sake) for more information about using and configuring `sake`, including how to register your own custom commands.
...
See the [`PolyCommand` documentation](/developer_guides/cli/polycommand) for more details about the `PolyCommand` API.
...
With the changes to `sake` come changes to the way `dev/*` actions are handled, both in the browser and in CLI. Most of these are now subclasses of the new [`DevCommand`](api:SilverStripe\Dev\Command\DevCommand) class which is itself a subclass of [`PolyCommand`](api:SilverStripe\PolyExecution\PolyCommand).
...
<details>
<summary>Click to see the diffs</summary>
...
You can use the [`scaffold_cms_fields_settings`](api:SilverStripe\ORM\DataObject->scaffold_cms_fields_settings) configuration property to change which fields are being scaffolded. See the [scaffolding](/developer_guides/model/scaffolding/#scaffolding-options) section for more details about using these options.
...
In Silverstripe CMS 5 [`CanonicalURLMiddleware`](api:SilverStripe\Control\Middleware\CanonicalURLMiddleware) only runs in production by default. This lead to issues with some functionality behaving differently in production environments compared to development. Silverstripe 6.0 changes this default to run the rules in `dev`, `test`, and `live` by default.
...
- Native indexed PHP arrays can now be passed into templates and iterated over with `<% loop $MyArray %>`.
  - Under the hood they are wrapped in [`ArrayList`](api:SilverStripe\Model\List\ArrayList), so you can get the count using `$MyArray.Count` and use `<% if $MyArray %>` as a shortcut for `<% if $MyArray.Count %>`.
  ...
  - Because arrays are no longer cast to a `DBField`, calling a `DBField` formatter on a getter that returns an array without a `$casting` entry now renders an empty string. For example, `$MyGetter.JSON` rendered JSON in CMS 5 but renders nothing in CMS 6. Return the encoded string from a separate getter instead (e.g. `$MyGetterJSON`), or declare a `$casting` type for the getter. Note that a getter with a `$casting` type can no longer be looped over.
...
- Values from template variables are passed into functions when used as arguments
  - For example, `$doSomething($Title)` will pass the value of the `Title` property into the `doSomething()` method. Previously `"$Title"` would be passed as a string. See [template syntax](/developer_guides/templates/syntax/#variables) documentation for more details.
...
- Cloning a [`Form`](api:SilverStripe\Forms\Form) will now recursively clone its validator, fields list, and actions list. This means they're completely independent, and updating the clone won't affect the original form.
- You can now set the `samesite` attribute for all cookies individually. See [secure sessions and cookies](/developer_guides/security/secure_coding/#secure-sessions-and-cookies) for details.
- The old convoluted way to add a history tab to an edit form by updating GraphQL queries and injecting react components no longer works. Instead, just add a [`HistoryViewerField`](api:SilverStripe\VersionedAdmin\Forms\HistoryViewerField) to your form.
...
[`Validator`](api:SilverStripe\Forms\Validation\Validator), its subclasses, and some related classes have been renamed:
...
|`SilverStripe\Security\PasswordValidator`|[`SilverStripe\Security\Validation\RulesPasswordValidator`](api:SilverStripe\Security\Validation\RulesPasswordValidator) (see [changes to password validation](#password-validation))|
...
|Old FQCN|New FQCN|
|---|---|
|`DNADesign\Elemental\TopPage\DataExtension`|[`DNADesign\Elemental\Extensions\TopPageElementExtension`](api:DNADesign\Elemental\Extensions\TopPageElementExtension)|
|`DNADesign\Elemental\TopPage\FluentExtension`|[`DNADesign\Elemental\Extensions\TopPageFluentElementExtension`](api:DNADesign\Elemental\Extensions\TopPageFluentElementExtension)|
|`DNADesign\Elemental\TopPage\SiteTreeExtension`|[`DNADesign\Elemental\Extensions\TopPageSiteTreeExtension`](api:DNADesign\Elemental\Extensions\TopPageSiteTreeExtension)|

The following miscellaneous classes and interfaces were also renamed:

|Old FQCN|New FQCN|
|---|---|
|`SilverStripe\CMS\Controllers\LeftAndMainPageIconsExtension`|[`SilverStripe\CMS\Controllers\LeftAndMainRecordIconsExtension`](api:SilverStripe\CMS\Controllers\LeftAndMainRecordIconsExtension)|
|`SilverStripe\CMS\Model\CurrentPageIdentifier`|[`SilverStripe\CMS\Model\CurrentRecordIdentifier`](api:SilverStripe\CMS\Model\CurrentRecordIdentifier)|
|`SilverStripe\Dev\Constraint\ViewableDataContains`|[`SilverStripe\Dev\Constraint\ModelDataContains`](api:SilverStripe\Dev\Constraint\ModelDataContains)|
|`SilverStripe\Dev\Validation\DatabaseAdminExtension`|[`SilverStripe\Dev\Validation\DbBuildExtension`](api:ilverStripe\Dev\Validation\DbBuildExtension)|
|`SilverStripe\Logging\HTTPOutputHandler`|[`SilverStripe\Logging\ErrorOutputHandler`](api:SilverStripe\Logging\ErrorOutputHandler)|
...
On [`RememberLoginHash`](api:SilverStripe\Security\RememberLoginHash) the `onAfterRenewToken` extension hook has been renamed to `onAfterRenewSession` - see [remember me token rotation](#remember-me-token-rotation) for details.
...
> [!IMPORTANT]
> As part of this change, [`FormField::validate()`](api:SilverStripe\Forms\FormField::validate()) now returns a [`ValidationResult`](api:SilverStripe\Core\Validation\ValidationResult) object where it used to return a boolean. The `$validator` parameter has also been removed.
>
...
- [`FormField::getValue()`](api:SilverStripe\Forms\FormField::getValue()) which usually returns an unmodified version of the value
- [`FormField::getFormattedValue()`](api:SilverStripe\Forms\FormField::getFormattedValue()) which is intended to be modified with things like localisation formatting and will be displayed to users
- [`FormField::dataValue()`](api:SilverStripe\Forms\FormField::dataValue()) which represents the value as passed into a record when [`saveInto()`](api:SilverStripe\Forms\FormField::saveInto()) is called. Usually this is the same as `getValue()`.
...
- [`DataObject::validate()`](api:SilverStripe\ORM\DataObject::validate()) now has an explicit [`ValidationResult`](api:SilverStripe\Core\Validation\ValidationResult) return type.
...
- Changed return type for method [`VirtualPage::CMSTreeClasses()`](api:SilverStripe\CMS\Model\VirtualPage::CMSTreeClasses()) from dynamic to `string`
