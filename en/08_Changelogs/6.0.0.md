...
- [Other changes](#other-changes)
  ...
  - [Session cookie changes](#session-cookie-changes)
...
Session cookies have been updated to be more secure. Regular cookies have not been changed.

The default value of [`Session.cookie_secure`](api:SilverStripe\Control\Session->cookie_secure) has been changed from `false` to `true`. This value corresponds with the cookie `Secure` attribute and also what will now happen is a seperate session cookie `SECSESSID` will be created and used for secure HTTPs connections as opposed to the regular session cookie `PHPSESSID` which is now only used for insecure HTTP connections.

The default value of [`Session.cookie_samesite`](api:SilverStripe\Control\Session->cookie_samesite) has been changed from `Lax` to `Strict`. This value corresponds to the cookie `SameSite` attribute. This has been done as a security measure to help prevent cross-site attacks.
