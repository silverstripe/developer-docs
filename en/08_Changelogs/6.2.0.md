...
- When attempting to generate a CMS edit link for an elemental block that is not inline-editable, or an inline block with a direct link, a `RuntimeException` will now be thrown if the parent `DataObject` is not a `SiteTree` and does not provide a valid `getCMSEditLink()`. This change ensures that elemental blocks consistently have a navigable path to their parent's edit form within the CMS.
